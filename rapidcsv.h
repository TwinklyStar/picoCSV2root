...
namespace rapidcsv
{
  ...
  public:
    ...
    int GetColumnIdx(const std::string& pColumnName) const
    {
      ...
      // **** Add by Meng Lyu on Dec. 26, 2023 ****
      void SetStartLineNum(int num){
        // jump a certain number of lines after the column header (default: 2)
          start_line = num;
      }
      // ******************************************
    ...
    std::vector<T> GetColumn(const size_t pColumnIdx) const
    {
      ...
      for (auto itRow = mData.begin() + start_line; itRow != mData.end(); ++itRow)
      {
        if (std::distance(mData.begin(), itRow) > mLabelParams.mColumnNameIdx)
        {
          ...
          {
            T val;
//            std::cout << itRow->at(dataColumnIdx);
